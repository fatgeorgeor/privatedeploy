...
def mount(
    ...
    try:
        LOG.debug('wuxingyidebug: Mounting %s on TEMP LOCATION %s with options %s', dev, path, options)
        command_check_call(
            [
                'mount',
                '-t', fstype,
...
def mount_activate(
    ...
    cluster = None
    try:
        LOG.info('wuxingyidebug: going to activate path %s' % path)
        ...
        try:
            ...
            LOG.info('src_dev is %s, dst_dev is %s' % (src_dev, src_dev))
            if src_dev == dst_dev:
                active = True
            else:
        ...
        if active:
            LOG.info('%s osd.%s already mounted in position; unmounting ours.'
                     % (cluster, osd_id))
            unmount((STATEDIR + '/osd/{cluster}-{osd_id}').format(cluster=cluster, osd_id=osd_id))
        elif other:
            unmount((STATEDIR + '/osd/{cluster}-{osd_id}').format(cluster=cluster, osd_id=osd_id))
            LOG.info('another %s osd.%s already mounted in position '
